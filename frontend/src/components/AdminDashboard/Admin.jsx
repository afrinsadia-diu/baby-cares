    ...
    useEffect(() => {
        ...
        if (toggleView.manageProducts) {
            fetch(`${process.env.REACT_APP_BACKEND_URL}/products`, {
                ...
                .then((res) => res.json())
                .then((data) => {
                    setmanageProducts(data.reverse());
                });
        }
    }, [toggleView.manageProducts, toggleView.managemyOrder, token, user.email]);

    // user can delete their own Order and admin also can delete any order
    const handleDeleteEvent = (id) => {
        ...
        fetch(`${process.env.REACT_APP_BACKEND_URL}/order/${id}`, {
            ...
            .then((result) => {
                ...
                if (result) {
                    const newOrders = managemyOrder.filter((order) => order._id !== id);
            ...
            <div className="vn-admin-dashboard px-lg-5 px-0">
                <div className="row">
                    <div className="col-lg-3 admin_wrapper">
                        <div className="admin-controls py-3 d-flex flex-lg-column">
                            ...
                            <button
                                ...
                                style={toggleView.showAddProduct ? style.primary : style.default}
                            >
                                <img src={plus} style={{ maxWidth: '17px' }} alt="" />
                            ...
                            <button
                                ...
                                style={toggleView.makeAdmin ? style.primary : style.default}
                            >
                                <img src={plus} style={{ maxWidth: '17px' }} alt="" />
                    ...
                    <div className="col-lg-9">
                        {/* admin can manage all order using this component */}
                        {toggleView.managemyOrder && (
                            <ManageOrder orders={managemyOrder} deleteHandler={handleDeleteEvent} />
                        )}
                        {/* admin can manage all Products using this component */}
                        ...
                        {/* admin can add new products using this component */}
                        {toggleView.showAddProduct && <AddService />}
                        {/* admin can make new admin using this component */}
                        {toggleView.makeAdmin && <MakeAdmin />}
                    </div>
                </div>
            </div>
        </div>
